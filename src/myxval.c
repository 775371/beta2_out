...
{   Rprintf("myxval.c\n");
    ...
    for (xgroup = 0; xgroup < n_xval; xgroup++) {
        ...
        if (split_Rule == 1) {
            ...
            // ct:
            (*ct_eval) (k, ct.ytemp, xtree->response_est, xtree->responses_est, xtree->controlMean, xtree->treatMean,
        ...
        for(i = k; i < ct.n; i++) {
            ...
            if (crossmeth == 1) {
                ...
                //CT- honest
                CTH_rundown(xtree, j, cp, xpred, xpreds, xtemp, k, cv_alpha, cv_eta, xtrain_to_est_ratio, ct.propensity);
